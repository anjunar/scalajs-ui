...
/** One column's resize constraints, as seen by a [[CustomColumnResizePolicy]]. A public mirror of
  * the internal `TableColumnLayout.Column` contract -- that object stays `private[table]`, since
  * it is an implementation detail shared by layout, pointer gestures and the imperative API, not a
...
/** The inputs to one custom-policy invocation.
  ...
  * size change or a column list change, the same event [[TableColumnLayout.layout]] handles for
...
/** A pluggable alternative to the seven built-in [[ColumnResizePolicy]] strategies (C05). Must
  * return a vector the same length as `request.columns`; anything else is rejected and the
  * previous widths are kept, matching the atomic-or-nothing contract the built-in policies already
  * have. Returned widths are still clamped to each column's own min/max, so a policy cannot violate
