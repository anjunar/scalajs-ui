...
/** Read-only cell state (D05) -- the same `index`/`empty`/`selected`/`focused`/`editing` properties
  * a `TableCell` subclass's own `renderContent` already reaches via `this`, projected for the
  * lighter-weight `cell`/`valueCell` bridge renderers, which only received the bare item before
  * this. Rendering and component ownership remain in Scala, the same split
