...
export const entries: readonly CatalogEntry[] = [
  ...
  catalogEntry(i18n`Dragging the "Book" group's own edge resizes Title and Author together, spilling into Year only once both are at their own limit.`, { de: "Das Ziehen der eigenen Kante der Gruppe \"Book\" verändert Titel und Autor gemeinsam und greift erst auf Jahr über, wenn beide an ihrer eigenen Grenze angekommen sind." }),
  catalogEntry(i18n`The Year cell highlights itself when it is focused or selected (D05), using the same per-cell state a custom row already gets for the whole row.`, { de: "Die Jahr-Zelle hebt sich selbst hervor, wenn sie fokussiert oder ausgewählt ist (D05) -- mit demselben Zellzustand, den eine eigene Zeile schon für sich als Ganzes bekommt." }),
  catalogEntry(i18n`Hover a row for its own tooltip (V07): a plain overlay anchored to the row, the same recipe as a column header's own menu.`, { de: "Eine Zeile zeigt beim Hover ihren eigenen Tooltip (V07): ein einfaches Overlay, an die Zeile geankert -- dasselbe Rezept wie das eigene Menü eines Spaltenkopfs." }),
  ...
  catalogEntry(i18n`Toggle fixed / variable row heights`, { de: "Feste/variable Zeilenhöhen umschalten" }),
  catalogEntry(i18n`A longer catalogue description that wraps in narrow columns.`, { de: "Eine längere Katalogbeschreibung, die in schmalen Spalten umbricht." }),
