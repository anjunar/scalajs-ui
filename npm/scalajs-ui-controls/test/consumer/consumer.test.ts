...
describe("typechecking a consumer", () => {
  it("resolves all three packages under --strict", () => {
    ...
    const source = [
      ...
      'import { carousel, checkBoxColumn, choiceBoxColumn, column, columnGroup, comboBoxColumn, dataGrid, progressBarColumn, remoteSource, tab, tableView, tabs, textFieldColumn, virtualList } from "@anjunar/scalajs-ui-controls";',
      ...
      '    tableView(editableRows, [textFieldColumn("Name", row => row.name, { editOnBlur: blur }), checkBoxColumn("Active", row => row.active), choiceBoxColumn("Role", row => row.name, ["Ada", "Grace"]), comboBoxColumn("Owner", row => row.name, ["Ada", "Grace"]), progressBarColumn("Progress", () => 0.5)], { editable: true });',
