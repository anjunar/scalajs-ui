...
describe("typechecking a consumer", () => {
  it("resolves all three packages under --strict", () => {
    ...
    const source = [
      ...
      'import type { ColumnDef, RemoteSource, TableDirection, TablePosition, TableSort, TableViewHandle, TableViewOptions, TableRowContext, TableSelectionMode, TextFieldBlurPolicy } from "@anjunar/scalajs-ui-controls";',
      "",
      ...
      "    const direction: TableDirection = \"rtl\"; const options: TableViewOptions<Row> = { direction, crawlable: true, crawlId: \"t\", cellSelectionEnabled: true, rowKey: row => row.name, row: (row: TableRowContext<Row>) => { const name: string | undefined = row.item.get?.name; row.renderCells(); } };",
