...
| C09 | Header-Grafik, Sortierdarstellung, Kontextmenü | Vorhanden | `headerCell`/`sortIndicator` (Scala und TypeScript) ersetzen den Default-Headertext bzw. den CSS-Only-Sortierpfeil durch komponierten Inhalt, gespeist aus dem reaktiven Sortierzustand (`sorted`/`ascending`/`priority`). Ein Kontextmenü ist bewusst keine dritte Property: `headerCell` ist echte Komposition und erreicht damit dasselbe öffentliche `overlay`, das jedes andere Dropdown/Popover schon nutzt, per `when(...)` geschaltet – keine zweite Popup-Engine. M5/M6. |
...
Die Bridge liest `headerCell: () => void` und `sortIndicator: (state) => (scope) => void` über den bestehenden `ControlFactories.slotBody`-Umwandlungspfad; `sortIndicator`s Zustand wird einmal pro Änderung als `js.Dynamic`-Literal projiziert und über `ReadOnlyPropertyHandle` an TypeScript gereicht, analog zu `visible`/`resizable`. TypeScript exportiert dafür den typisierten `SortIndicatorState`.

**Kontextmenü, bewusst keine dritte Property:** Ein Header-Rechtsklick-/Kontextmenü ist kein eigener Slot neben `headerCell`/`sortIndicator`. `headerCell` ist echte DSL-Komposition, kein enges „Graphic-Node"-Property; eine App, die ein Kontextmenü will, kommt dort genauso hin wie jede andere Komponente: `on("contextmenu") { ... }` innerhalb des Rumpfs, gefolgt von `when(offenProperty) { overlay(...) { ... } }` – demselben öffentlichen, ankerfolgenden Overlay, das `ComboBox`s eigenes Dropdown und C08s Spaltenmenü bereits verwenden, nur ohne dessen fest generierte Checkbox-Einträge. Keine zweite Popup-Engine, dieselbe Entscheidung wie bei C08. Am 17.09.2026 in der Scala-Demo real verdrahtet und im Browser geprüft: Rechtsklick auf den Titel-Header öffnet ein Menü mit „Auto-fit column"/„Hide column"; `event.preventDefault()` unterdrückt das native Kontextmenü zuverlässig, Escape und ein Menüpunkt-Klick schließen es, „Hide column" setzt tatsächlich `titleColumn.visible = false`. TypeScript hat dieselben drei Bausteine (`on`, `when`, `overlay`) bereits öffentlich exportiert; ein äquivalentes Rezept steht im [Paket-README](npm/scalajs-ui-controls/README.md#column-header-content).
...
- [x] M5/M6: Header-Grafik und Sortierdarstellung (C09) sind umgesetzt; Kontextmenü erreicht bewusst dasselbe öffentliche `overlay` statt einer dritten Property, real verdrahtet und im Browser geprüft.
...
Abschluss des Kontextmenü-Teils von C09 am 17.09.2026: keine neue Framework-API, also kein neuer Scala-/Bridge-Testfall nötig – `headerCell`, `on(...)`, `when(...)` und `overlay(...)` sind alle bereits an anderer Stelle abgedeckt (u. a. C09s eigener `headerCell`-Fall, C08s Spaltenmenü als Overlay-Nutzer). Stattdessen real in der Scala-Demo verdrahtet (Titel-Header, "Auto-fit column"/"Hide column") und direkt im Browser prüft: ein `contextmenu`-Event auf dem Header setzt `defaultPrevented=true` (natives Menü unterdrückt) und öffnet `.table-demo__header-menu`; Escape schließt es; ein Klick auf „Hide column" setzt `titleColumn.visible = false` und die Titelspalte verschwindet tatsächlich aus der Tabelle. `scalajs-ui-demo`-Testlauf (17 Tests) weiterhin grün.
